...
function parseBinaryXVIZ(arrayBuffer) {
  if (checkMagic(arrayBuffer, {
    magic: _protobufSupport.MAGIC_PBE1
  })) {
    var data = parsePBEXVIZ(arrayBuffer);
    return data;
  }
...
function parsePBEXVIZ(arrayBuffer) {
  ...
  switch (envelope.type) {
    case 'xviz/metadata':
      var tmpMeta = _protobufSupport.XVIZ_PROTOBUF_MESSAGE.Metadata.decode(envelope.data.value);
    ...
    case 'xviz/state_update':
      var tmpState = _protobufSupport.XVIZ_PROTOBUF_MESSAGE.StateUpdate.decode(envelope.data.value);

      xviz.data = postProcessProtobuf(tmpState);
      break;

    default:
      throw new Error("Unknown Message type ".concat(envelope.type));
  }
